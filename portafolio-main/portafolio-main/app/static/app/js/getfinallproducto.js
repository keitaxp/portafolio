let rutaget = "http://127.0.0.1:8001/api/v1/producto/";
...
$.get(rutaget, function(leerRuta){
    ...
    leerRuta.forEach(function(params){
        ...
        <td>${params.id_producto}</td>
        <td>${params.id_proveedor_id}</td>
        <td>${params.nombre_producto}</td>
        <td>${params.descripcion}</td>
        <td>
            <a href="#"><button class="btn btn-primary" id="btnEditar${cont}" onclick="eliminar(${params.id_producto})" /i> Borrar </button>
        </td>
        </tr>`
        cont++;
    ...
    function eliminar(id){
        let rutaDL = "http://127.0.0.1:8001/api/v1/producto/" + id;
